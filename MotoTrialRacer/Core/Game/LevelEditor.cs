...
{
    /// <summary>
    /// The class that defines the level editor. Derives from Level.
    /// </summary>
    class LevelEditor : Level
    {
        ...
        /// <summary>
        /// Stops testing and returns to the modifying view.
        /// </summary>
        ...
        /// <summary>
        /// Saves the level definitions to the user save directory.
        /// </summary>
        public bool SaveToFile(String name)
        {
            ...
            // Protect built-in level names and prevent overwriting existing custom levels
            if (File.Exists(filePath) || name == "level1" || name == "level2" || name == "level3")
                return false;

            using (StreamWriter writer = new StreamWriter(filePath))
            {
                writer.WriteLine(string.Format("start:{0}:{1}",
                    bikePosDestination.X, bikePosDestination.Y));

                foreach (var c in components)
                {
                    string s = string.Format("{0}:{1}:{2}:{3}",
                        c.Name, c.Pos.X, c.Pos.Y, c.Angle);
                    writer.WriteLine(s);
                }
            }
            return true;
        ...
        // Empty overrides: bike behaves differently in the editor
        public override void StopBikeMotor() { }
        public override void ResetBike() { }

        /// <summary>
        /// Draws all buttons and level components in the level editor.
        /// </summary>
